...
export function loadSQLWithParams(relativePath: string, params: Record<string, string>): string {
  ...
  for (const [key, value] of Object.entries(params)) {
    ...
    if (containsSuspiciousPatterns(value)) {
      ...
      console.warn(
        `[SECURITY] Suspicious SQL pattern detected in parameter '${key}' for SQL file '${relativePath}'. ` +
        `Value contained patterns that may indicate SQL injection attempt.`
      );
    ...
    // Use simple string replacement since placeholder format is controlled
...
function containsSuspiciousPatterns(value: string): boolean {
  const suspiciousPatterns = [
    ...
    /\s--\s/,                                // SQL line comments (with spaces)
    /--$/m,                                  // SQL line comments at end of line
    /\/\*/,                                  // Block comment start
